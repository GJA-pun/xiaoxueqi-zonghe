      ...
      <c:forEach items="${ model.data }" var="o" >
          ...
          <td>
          	<div class="button-group">
				<a class="button border-main" href="javascript:void(0)" onclick="update(this)">
					<span class="icon-edit"></span> 修改</a>
				<a style="display: none" class="button border-main" href="javascript:void(0)" onclick="save(this)">
...
$(document).ready(function(){
	$(".type").dblclick(function(){
		...
		$select.dblclick(function(){return false;});
...
// 					$select.append("<option>"+list[i].name+"</option>");
				}
				$select.find("option[text='"+txt+"']").attr("selected",true);
// 				$select.text(txt);
				$select.change(function(){
					var v = $(this).val();
					var t = $(this).find("option[value='"+ v +"']").text();
					
					updateGtid(v, $select, txt, $(this));
				})
			}
		})
...
function updateGtid(gtid, $select, txt, item){
	// 获取商品的id 
	...
	//2 ajax请求
	$.ajax({
		url:"rest/goods/update/type",
		type:"post",
		data:JSON.stringify({"goodsid":goodsid,"gtid":gtid}),
		contentType:"application/json",
		success:function(r){
			var $d = dialog({	// 使用第三方库，创建对话框
				title: "修改确认",
				content: "你确认要修改商品类型吗?", // 可以放入html代码
				okValue: "确定",
				ok: function(){
					var v = item.val();
					var t = item.find("option[value='"+ v +"']").text();
					$select.parent().html(t);
				},
				cancelValue: "取消",
				cancel: function(){
					$select.parent().html(txt);
				}
			});
			$d.showModal(); // 模式对话框，背景无法操作
		}
	})
	//3 增加更新类别方法
	//4 实现方法
...
function save(t){
	...
	var $d = dialog({	// 使用第三方库，创建对话框
		title: "修改确认",
		content: "你确认要修改商品数据吗?", // 可以放入html代码
		okValue: "确定",
		ok: function(){
			$.ajax({
				url: "admin/goods/update.php", // 最前面不要"/"
				type: "post",
				data: data,
				success: function(e){
